...
### `ap(fn)` - Apply Either-Wrapped Function
...
# Result: Success or error with message
...
**Rule of thumb**:
- Use **Either** when you need two distinct types (not just success/error) or custom error objects
- Use **Result** when you have simple success/error cases with error messages
